    ...
    if (empty($errors)) {
        ...
        if (!empty($_GET)) {
            if (!empty($_GET['postId'])) {
                $updateQuery = $db->prepare('UPDATE posts SET user_id=:userId, category_id=:categoryId, text=:text WHERE post_id =:postId;');
                $updateQuery->execute([
                    'userId' => 1,
...
//Load possible post for editing
if (!empty($_GET)) {
    if (!empty($_GET['postId'])) {
        $editQuery = $db->prepare('SELECT * FROM posts WHERE post_id=:post LIMIT 1');
        $editQuery->execute([
            ':post' => $_GET['postId']
        ]);
        if ($editQuery->rowCount() === 0) {
            echo 'Neplatné id příspěvku';
        } else {
            $post = $editQuery->fetch();
            $_POST['category'] = $post['category_id'];
    ...
    <form method="post">
        ...
        <div class="form-group">
            ...
            <textarea name="text" id="text" required
                      class="form-control <?php echo(!empty($errors['category']) ? 'is-invalid' : ''); ?>"><?php echo htmlspecialchars(@$_POST['text']) ?></textarea>
            <?php
