...
if (!empty($_SESSION['user_id']) && $_SESSION['admin']) {
    echo '<a href="categories.php" class="text-danger">Upravit kategorie</a>';
}
echo '</form>';
...
if (!empty($posts)) {
    ...
    foreach ($posts as $post) {
        ...
        if (!empty($_SESSION['user_id']) && ($_SESSION['admin'] || $_SESSION['user_id'] == $post['user_id'])) {
            echo ' - <a href="edit.php?id=' . $post['post_id'] . '" class="text-danger">upravit</a>';
            echo ' - <a href="delete.php?id=' . $post['post_id'] . '" class="text-danger">smazat</a>';
        }
